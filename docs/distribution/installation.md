...
```bash
pip install atlas-agent[openai,config]
pip install atlas-agent[full]
```

O pacote básico `atlas-agent` instala apenas o core. `full` é conveniência e
não é requisito arquitetural. Nenhuma instalação inicia cliente, servidor,
consumer, plugin ou chamada de rede.
